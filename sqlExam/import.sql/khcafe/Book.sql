...
--책이름을 7번부터 10번까지 '나디아 연대기'로 변경하고자 한다. (between활용)
UPDATE book SET title = '나디아 연대기' WHERE book_id BETWEEN 7 AND 10;

--UPDATE 를 이용해서 쿼리문을 변경하는 방법
UPDATE book 
SET title = 
    CASE 
        ...
        WHEN title = '바람의 나라' THEN '리그오브레전드'
        WHEN title = '미래의 선택' THEN '과거의 선택'
        WHEN title = '꽃을 보듯 너를 본다' THEN '꽃을 보듯 나를 본다'
        WHEN title = '아프니까 청춘이다' THEN '아프니까 아프다'
        WHEN title = '소나기' THEN '우박'
        WHEN title = '우리들의 행복한 시간' THEN '우행시'
        WHEN title = '몽실언니' THEN '몽쉘언니'
        ELSE title
    END;
